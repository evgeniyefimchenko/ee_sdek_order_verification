...
if ($_GET['access_code'] == $settings_addon['access_code']) {
	...
	foreach ($statuses as $key => $val) {
		if (mb_strlen($settings_addon[$val[0]['code']]) > 0 && array_key_exists($settings_addon[$val[0]['code']], $cscart_statuses_shipments)) { // Статусы отгрузок
		...
		if (mb_strlen($addon_order_code) > 0 && array_key_exists($addon_order_code, $cscart_statuses_orders)) { // Статусы заказов
			$order_id = db_fet_field('SELECT order_id FROM ?:shipment_items WHERE shipment_id = ?i', $key);
